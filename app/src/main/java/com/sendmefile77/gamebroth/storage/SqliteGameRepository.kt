...
import com.sendmefile77.gamebroth.model.ArtifactState
import com.sendmefile77.gamebroth.model.ClientProfile
...
import com.sendmefile77.gamebroth.model.FactionState
import com.sendmefile77.gamebroth.model.GameState
import com.sendmefile77.gamebroth.model.InventoryItem
import com.sendmefile77.gamebroth.model.PersonalPurchase
import com.sendmefile77.gamebroth.model.PreferenceStance
import com.sendmefile77.gamebroth.model.QuestState
import com.sendmefile77.gamebroth.model.QuestStatus
import com.sendmefile77.gamebroth.model.SecretState
...
import com.sendmefile77.gamebroth.model.GalleryFrame
...
class SqliteGameRepository(context: Context) : SQLiteOpenHelper(
    ...
    override fun saveState(state: GameState) {
        ...
        try {
            ...
            val gameValues = ContentValues().apply {
                ...
                put("world_seed", state.worldSeed)
                put("current_day", state.currentDay)
                put("establishment_id", e.id)
                put("establishment_name", e.name)
                put("establishment_level", e.level)
                put("treasury", e.treasury)
                put("debt", e.debt)
                put("public_reputation", e.publicReputation)
                put("heat", e.heat)
                put("luxury", e.luxury)
                put("secrecy", e.secrecy)
                put("arcane", e.arcane)
                put("political_influence", e.politicalInfluence)
            }
            db.insertWithOnConflict("game_state", null, gameValues, SQLiteDatabase.CONFLICT_REPLACE)
    ...
    override fun appendMemory(memory: StaffMemory) {
        writableDatabase.insertWithOnConflict("staff_memories", null, ContentValues().apply {
            ...
            put("category", memory.category); put("summary", memory.summary); put("importance", memory.importance)
            put("created_at", memory.createdAtEpochMs)
    ...
    override fun memoriesForStaff(staffId: String, limit: Int): List<StaffMemory> {
        ...
        return readableDatabase.query(
            "staff_memories",
            arrayOf("id", "staff_id", "day", "category", "summary", "importance", "created_at"),
            "staff_id = ?",
            arrayOf(staffId),
            null,
            null,
            "importance DESC, day DESC, created_at DESC",
            limit.toString(),
        ).use { cursor ->
            buildList {
                while (cursor.moveToNext()) add(
                    StaffMemory(
                        id = cursor.getString(0), staffId = cursor.getString(1), day = cursor.getInt(2),
                        category = cursor.getString(3), summary = cursor.getString(4), importance = cursor.getInt(5),
    ...
    override fun recentWorldEvents(limit: Int): List<WorldEvent> {
        ...
        return readableDatabase.query(
            "world_events",
            arrayOf("id", "day", "type", "summary", "payload", "created_at"),
            null,
            null,
            null,
            null,
            "day DESC, created_at DESC",
            limit.toString(),
        ).use { cursor ->
            buildList {
                while (cursor.moveToNext()) add(
                    WorldEvent(
                        id = cursor.getString(0), day = cursor.getInt(1), type = cursor.getString(2),
                        summary = cursor.getString(3), payload = cursor.getString(4), createdAtEpochMs = cursor.getLong(5),
                    ),
                )
            }
        }
    ...
    override fun saveDailyReport(report: DailyReport) {
        ...
        try {
            ...
            report.staffReports.forEach { sr ->
                ...
                sr.encounters.forEach { e -> insertEncounter(db, e) }
            }
            db.setTransactionSuccessful()
    ...
    private fun loadEncounters(db: SQLiteDatabase, day: Int, staffId: String): List<WorkEncounter> = db.query("work_encounters", null, "day = ? AND staff_id = ?", arrayOf(day.toString(),staffId), null, null, "id").use { c ->
        buildList { while(c.moveToNext()) {
    ...
    override fun saveVisualProfile(profile: VisualIdentityProfile) {
        writableDatabase.insertWithOnConflict("visual_identity", null, ContentValues().apply {
            ...
            put("locked", if (profile.locked) 1 else 0)
            put("age_band", profile.ageBand)
            put("build_desc", profile.build)
            put("skin_tone", profile.skinTone)
            put("hair", profile.hair)
            put("eyes", profile.eyes)
            put("face", profile.face)
            put("distinctive_marks", profile.distinctiveMarks.joinToString("|"))
            put("species_tokens", profile.speciesTokens.joinToString("|"))
            put("body_plan", profile.bodyPlan)
            put("wardrobe_tokens", profile.wardrobeTokens.joinToString("|"))
            put("style_tokens", profile.styleTokens.joinToString("|"))
            put("canonical_frame_id", profile.canonicalFrameId)
            put("updated_day", profile.updatedAtDay)
    ...
    override fun saveGalleryFrame(frame: GalleryFrame) {
        writableDatabase.insertWithOnConflict("gallery_frames", null, ContentValues().apply {
            ...
            put("seed", frame.seed); put("width", frame.width); put("height", frame.height)
            put("reference_frame_id", frame.referenceFrameId); put("profile_revision", frame.profileRevision)
            put("created_at", frame.createdAtEpochMs); put("is_canonical", if (frame.canonical) 1 else 0)
        }, SQLiteDatabase.CONFLICT_REPLACE)
    }
    ...
    override fun galleryForStaff(staffId: String, limit: Int): List<GalleryFrame> {
        ...
        return readableDatabase.query(
            "gallery_frames", null, "staff_id = ?", arrayOf(staffId), null, null,
            "day DESC, created_at DESC", limit.toString(),
    ...
    private fun createV3Tables(db: SQLiteDatabase) {
        db.execSQL("""CREATE TABLE IF NOT EXISTS visual_identity(
            ...
            eyes TEXT NOT NULL,
            face TEXT NOT NULL,
            distinctive_marks TEXT NOT NULL,
            species_tokens TEXT NOT NULL,
            body_plan TEXT NOT NULL,
            wardrobe_tokens TEXT NOT NULL,
            style_tokens TEXT NOT NULL,
            canonical_frame_id TEXT,
    ...
    private fun visualProfileFromCursor(c: android.database.Cursor) = VisualIdentityProfile(
        ...
        ageBand = c.string("age_band"), build = c.string("build_desc"), skinTone = c.string("skin_tone"),
        hair = c.string("hair"), eyes = c.string("eyes"), face = c.string("face"),
        distinctiveMarks = splitTokens(c.string("distinctive_marks")), speciesTokens = splitTokens(c.string("species_tokens")),
        bodyPlan = c.string("body_plan"), wardrobeTokens = splitTokens(c.string("wardrobe_tokens")),
        styleTokens = splitTokens(c.string("style_tokens")), canonicalFrameId = c.nullableString("canonical_frame_id"),
        updatedAtDay = c.int("updated_day"),
    )
    ...
    private fun galleryFrameFromCursor(c: android.database.Cursor) = GalleryFrame(
        ...
        localPath = c.string("local_path"), prompt = c.string("prompt"), negativePrompt = c.string("negative_prompt"),
        seed = c.nullableLong("seed"), width = c.int("width"), height = c.int("height"),
        referenceFrameId = c.nullableString("reference_frame_id"), profileRevision = c.int("profile_revision"),
    ...
    private fun loadState(): GameState? {
        ...
        val row = db.query("game_state", null, "id = 1", null, null, null, null).use { cursor ->
            ...
            GameState(
                ...
                establishment = EstablishmentState(
                    id = cursor.string("establishment_id"),
                    name = cursor.string("establishment_name"),
                    level = cursor.int("establishment_level"),
                    treasury = cursor.long("treasury"),
                    debt = cursor.long("debt"),
                    publicReputation = cursor.int("public_reputation"),
                    heat = cursor.int("heat"),
                    luxury = cursor.int("luxury"),
                    secrecy = cursor.int("secrecy"),
                    arcane = cursor.int("arcane"),
                    politicalInfluence = cursor.int("political_influence"),
                ),
                staff = emptyList(),
            )
        }
        return row.copy(
            staff = loadStaff(db),
            quests = loadQuests(db),
            factions = loadFactions(db),
            artifacts = loadArtifacts(db),
            secrets = loadSecrets(db),
        )
    }
    ...
    private fun insertStaff(db: SQLiteDatabase, member: StaffMember) {
        db.insertOrThrow("staff", null, ContentValues().apply {
            ...
            put("stress", member.stress); put("fatigue", member.fatigue); put("health", member.health)
            put("reputation", member.reputation); put("status", member.status.name)
        })
        member.traits.forEach { trait ->
            db.insertOrThrow("staff_traits", null, ContentValues().apply { put("staff_id", member.id); put("trait", trait) })
        }
        member.skills.values.forEach { skill ->
            db.insertOrThrow("staff_skills", null, ContentValues().apply {
                put("staff_id", member.id); put("code", skill.code); put("level", skill.level); put("xp", skill.xp)
            })
        }
        member.preferences.forEach { (code, stance) ->
            db.insertOrThrow("staff_preferences", null, ContentValues().apply { put("staff_id", member.id); put("code", code); put("stance", stance.name) })
        }
        member.inventory.forEach { item ->
            db.insertOrThrow("inventory", null, ContentValues().apply {
                put("item_id", item.id); put("staff_id", member.id); put("name", item.name); put("quantity", item.quantity)
            })
            item.tags.forEach { tag ->
                db.insertOrThrow("inventory_tags", null, ContentValues().apply {
                    put("staff_id", member.id); put("item_id", item.id); put("tag", tag)
                })
            }
        }
    }

    private fun loadStaff(db: SQLiteDatabase): List<StaffMember> = db.query("staff", null, null, null, null, null, "name").use { cursor ->
        buildList {
            while (cursor.moveToNext()) {
                val id = cursor.string("id")
                val traits = stringColumn(db, "staff_traits", "trait", "staff_id", id)
                val skills = db.query("staff_skills", null, "staff_id = ?", arrayOf(id), null, null, "code").use { skillCursor ->
                    buildMap {
                        while (skillCursor.moveToNext()) {
                            val code = skillCursor.string("code")
                            put(code, SkillProgress(code, skillCursor.int("level"), skillCursor.int("xp")))
                        }
                    }
                }
                val preferences = db.query("staff_preferences", null, "staff_id = ?", arrayOf(id), null, null, "code").use { pc ->
                    buildMap { while (pc.moveToNext()) put(pc.string("code"), PreferenceStance.valueOf(pc.string("stance"))) }
                }
                val inventory = db.query("inventory", null, "staff_id = ?", arrayOf(id), null, null, "name").use { itemCursor ->
                    buildList {
                        while (itemCursor.moveToNext()) {
                            val itemId = itemCursor.string("item_id")
                            add(InventoryItem(
                                id = itemId,
                                name = itemCursor.string("name"),
                                quantity = itemCursor.int("quantity"),
                                tags = stringColumn(db, "inventory_tags", "tag", "staff_id = ? AND item_id = ?", arrayOf(id, itemId)),
                            ))
                        }
                    }
                }
                add(StaffMember(
                    ...
                    species = cursor.string("species"),
                    ageYears = cursor.int("age_years"),
                    level = cursor.int("level"),
                    xp = cursor.int("xp"),
                    personalMoney = cursor.long("personal_money"),
                    loyalty = cursor.int("loyalty"),
                    stress = cursor.int("stress"),
                    fatigue = cursor.int("fatigue"),
                    health = cursor.int("health"),
                    reputation = cursor.int("reputation"),
                    status = StaffStatus.valueOf(cursor.string("status")),
                    traits = traits,
                    skills = skills,
    ...
    private fun loadQuests(db: SQLiteDatabase): List<QuestState> = db.query("quests", null, null, null, null, null, "id").use { cursor ->
        buildList { while (cursor.moveToNext()) add(QuestState(cursor.string("id"), cursor.string("title"), cursor.string("summary"), QuestStatus.valueOf(cursor.string("status")), cursor.int("progress"), cursor.int("target"))) }
    }

    private fun loadFactions(db: SQLiteDatabase): List<FactionState> = db.query("factions", null, null, null, null, null, "id").use { cursor ->
        buildList { while (cursor.moveToNext()) add(FactionState(cursor.string("id"), cursor.string("name"), cursor.int("relation"), cursor.int("influence"), cursor.int("attention"))) }
    }

    private fun loadArtifacts(db: SQLiteDatabase): List<ArtifactState> = db.query("artifacts", null, null, null, null, null, "id").use { cursor ->
        buildList {
            while (cursor.moveToNext()) {
                val id = cursor.string("id")
                add(ArtifactState(id, cursor.string("name"), cursor.int("charges"), cursor.int("max_charges"), stringColumn(db, "artifact_tags", "tag", "artifact_id", id)))
            }
        }
    }

    private fun loadSecrets(db: SQLiteDatabase): List<SecretState> = db.query("secrets", null, null, null, null, null, "id").use { cursor ->
        buildList {
            while (cursor.moveToNext()) add(SecretState(
                id = cursor.string("id"), title = cursor.string("title"),
                ownerNpcId = cursor.nullableString("owner_npc_id"), leverage = cursor.int("leverage"), exposed = cursor.int("exposed") != 0,
            ))
        }
    }

    private fun stringColumn(db: SQLiteDatabase, table: String, column: String, key: String, value: String): Set<String> =
        stringColumn(db, table, column, "$key = ?", arrayOf(value))

    private fun stringColumn(db: SQLiteDatabase, table: String, column: String, selection: String, args: Array<String>): Set<String> =
        db.query(table, arrayOf(column), selection, args, null, null, column).use { cursor ->
