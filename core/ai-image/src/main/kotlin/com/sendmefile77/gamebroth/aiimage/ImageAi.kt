...
object VisualPromptBuilder {
    fun build(
        ...
        val roleText = when (mode) {
            FramePromptMode.PORTRAIT -> "SINGLE CONTINUOUS IMAGE. Exactly one fictional adult woman, upright and right-side-up. FULL-BODY CHARACTER REFERENCE PORTRAIT, HEAD TO TOE. The entire body MUST fit inside the frame from the top of the hair to both feet, with comfortable margins above the head and below the feet. The figure should occupy about eighty percent of the vertical canvas, centered and balanced. Her complete face must be clearly readable, both eyes visible, hairstyle readable, shoulders and torso fully visible, arms naturally placed, legs shown in normal proportion, both feet visible on the floor. Neutral relaxed standing pose, front or gentle three-quarter orientation, normal eye-level perspective, simple unobtrusive background. Balanced proportions: the face remains an important identity anchor while the feet are visible but NOT emphasized. No body part may dominate the frame. This is a full-body identity portrait, not a body-part study and not an action scene."
        ...
        val modeNegative = when (mode) {
            FramePromptMode.PORTRAIT -> "busy action scene, crowd, multiple people, two people, man, male, extreme viewpoint, extreme perspective, extreme foreshortening, feet close-up, foot close-up, soles close-up, feet only, legs only, legs close-up, body-part crop, disembodied limbs, headless, faceless, face outside frame, cropped head, cropped face, cropped feet, feet outside frame, upside-down person, inverted body, back-facing portrait, low-angle foot focus, floor-level viewpoint, giant feet, tiny head, distorted body proportions, fisheye perspective"
        ...
        val (width, height) = when (mode) {
            FramePromptMode.PORTRAIT -> 768 to 1152
            FramePromptMode.STORY, FramePromptMode.REPORT -> 768 to 1024
        }
        return StaffFramePrompt(role, mode, prompt, "$commonNegative, $modeNegative", width, height)
    }
}
